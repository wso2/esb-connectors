...
public final class SalesforceUtil {
    ...
    public void addSobjects(String strOperation, String strParamName,
                            MessageContext synCtx, SynapseLog synLog, String strExternalId) {
        SOAPEnvelope envelope = synCtx.getEnvelope();
        OMFactory fac = OMAbstractFactory.getOMFactory();
        SOAPBody body = envelope.getBody();
        Iterator<OMElement> bodyChildElements = body
                .getChildrenWithLocalName(strOperation);
        if (bodyChildElements.hasNext()) {
            try {
                OMElement bodyElement = bodyChildElements.next();
                if (strExternalId != null) {
                    OMNamespace omNs = fac.createOMNamespace(
                            "urn:partner.soap.sforce.com", "urn");
                    OMElement value = fac.createOMElement(
                            "externalIDFieldName", omNs);
                ...
                OMNamespace omNsurn = fac.createOMNamespace(
                        "urn:partner.soap.sforce.com", "urn");
                OMNamespace omNsurn1 = fac.createOMNamespace(
                        "urn:sobject.partner.soap.sforce.com", "urn1");
                // Loops sObject
                while (sObject.hasNext()) {
                    OMElement currentElement = sObject.next();
                    OMElement newElement = fac.createOMElement("sObjects",
                                                               omNsurn);
                    // Add Object type
                    if (strType != null) {
                    ...
                    // Add the fields
                    Iterator<OMElement> sObjectFields = currentElement
                            .getChildElements();
                    while (sObjectFields.hasNext()) {
                        OMElement sObjectField = sObjectFields.next();
                        tmpElement = fac.createOMElement(
                                sObjectField.getLocalName(), omNsurn1);
                        tmpElement.addChild(fac.createOMText(sObjectField
                                                                     .getText()));
                        newElement.addChild(tmpElement);
                    }
                ...
                synLog.error("Saleforce adaptor - error injecting sObjects to payload : "
                             + e);
            }
        }
    }

    public void addIds(String strOperation, String strParamName,
        ...
        if (bodyChildElements.hasNext()) {
            try {
                ...
                String strSobject = (String) ConnectorUtils
                        .lookupTemplateParamater(synCtx, strParamName);
                OMElement sObjects = AXIOMUtil.stringToOM(strSobject);
                Iterator<OMElement> sObject = sObjects.getChildElements();
                OMNamespace omNsurn = fac.createOMNamespace(
                        "urn:partner.soap.sforce.com", "urn");
                ...
                while (sObject.hasNext()) {
                    ...
                    // Add the fields
                    newElement.addChild(fac.createOMText(currentElement
                                                                 .getText()));
                    bodyElement.addChild(newElement);
                }
            } catch (Exception e) {
    ...
    public void addAttachementSobject(String strOperation, String strParamName,
                                      MessageContext synCtx, SynapseLog synLog,
        ...
        if (bodyChildElements.hasNext()) {
            try {
                ...
                String strSobject = strParamName;
                OMElement sObjects = AXIOMUtil.stringToOM(strSobject);
                Iterator<OMElement> sObject = sObjects.getChildElements();
                String strType = sObjects.getAttributeValue(new QName(
                        SalesforceUtil.SALESFORCE_CREATE_SOBJECTTYPE));
