...
//    }
    ...
    /**
     * Positive test case for createMultipleRecord method with mandatory parameters.
     */
    @Test(enabled = true, description = "salesforcerest {createMultipleRecord} integration test with mandatory parameters.")
    public void createMultipleRecordWithMandatoryParameters() throws IOException, JSONException {
        String methodName = "createMultipleRecord";
        RestResponse<JSONObject> esbRestResponse = sendJsonRestRequest(getProxyServiceURL(methodName), "POST", esbRequestHeadersMap, "createMultipleRecord.json");
        String apiEndPoint = connectorProperties.getProperty("apiUrl") + "/services/data/"+connectorProperties.getProperty("apiVersion")+"/composite/tree/"+connectorProperties.getProperty("sobject")+"/";
        RestResponse<JSONObject> apiRestResponse = sendJsonRestRequest(apiEndPoint, "GET", apiRequestHeadersMap);
        Assert.assertEquals(esbRestResponse.getHttpStatusCode(), 201);
        Assert.assertEquals(apiRestResponse.getHttpStatusCode(), 404);
        Assert.assertEquals(esbRestResponse.getBody().toString(), apiRestResponse.getBody().toString());
