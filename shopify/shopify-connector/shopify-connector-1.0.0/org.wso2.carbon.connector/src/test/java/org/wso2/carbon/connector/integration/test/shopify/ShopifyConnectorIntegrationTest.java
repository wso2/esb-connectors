...
public class ShopifyConnectorIntegrationTest extends ConnectorIntegrationTestBase {
    ...
    /**
     * Positive test case for getOrderById method with mandatory parameters.
     */
    @Test(groups = { "wso2.esb" }, dependsOnMethods={"testCreateOrderWithMandatoryParameters"}, description = "Shopify {getOrderById} integration test with mandatory parameters.")
    public void testGetOrderByIdWithMandatoryParameters() throws Exception {
    
        esbRequestHeadersMap.put("Action", "urn:getOrderById");
        RestResponse<JSONObject> esbRestResponse =
                sendJsonRestRequest(proxyUrl, "POST", esbRequestHeadersMap, "esb_getOrderById_mandatory.json");
        
        String apiEndPoint =
                connectorProperties.getProperty("apiUrl") + "/admin/orders/"
                        + connectorProperties.getProperty("orderId") + ".json";
        
        RestResponse<JSONObject> apiRestResponse = sendJsonRestRequest(apiEndPoint, "GET", apiRequestHeadersMap);
        
        Assert.assertEquals(esbRestResponse.getHttpStatusCode(), 200);
        Assert.assertEquals(apiRestResponse.getBody().getJSONObject("order").getString("name"), esbRestResponse.getBody().getJSONObject("order").getString("name"));
        Assert.assertEquals(apiRestResponse.getBody().getJSONObject("order").getString("email"), esbRestResponse.getBody().getJSONObject("order").getString("email"));
        Assert.assertEquals(apiRestResponse.getBody().getJSONObject("order").getString("created_at"), esbRestResponse.getBody().getJSONObject("order").getString("created_at"));
        
    }
    
    /**
     * Positive test case for getOrderById method with optional parameters.
     */
    @Test(groups = { "wso2.esb" }, dependsOnMethods={"testCreateOrderWithMandatoryParameters"}, description = "Shopify {getOrderById} integration test with optional parameters.")
    public void testGetOrderByIdWithOptionalParameters() throws Exception {
    
        esbRequestHeadersMap.put("Action", "urn:getOrderById");
        RestResponse<JSONObject> esbRestResponse =
                sendJsonRestRequest(proxyUrl, "POST", esbRequestHeadersMap, "esb_getOrderById_optional.json");
        
        String apiEndPoint =connectorProperties.getProperty("apiUrl") + "/admin/orders/"
                + connectorProperties.getProperty("orderId") + ".json";
        RestResponse<JSONObject> apiRestResponse = sendJsonRestRequest(apiEndPoint, "GET", apiRequestHeadersMap);
    
        Assert.assertEquals(esbRestResponse.getHttpStatusCode(), 200);
        
        Assert.assertNotSame(apiRestResponse.getBody().getJSONObject("order").has("id"), esbRestResponse.getBody().getJSONObject("order").has("id"));
        Assert.assertNotSame(apiRestResponse.getBody().getJSONObject("order").has("created_at"), esbRestResponse.getBody().getJSONObject("order").has("created_at"));
        
        Assert.assertTrue(esbRestResponse.getBody().getJSONObject("order").has("name"));
